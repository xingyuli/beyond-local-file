...
`daemon status` lists held copies. `daemon start` does not require ack (the resolve UI needs a running daemon). `daemon reload` prints a warning and requires ack; it does not require the user to restore or discard before the daemon runs. There is no held restore/discard shell: the status path is enough to copy by hand. Held copies appear in the resolve UI nav (0024); inspect and restore of held bytes stay later.
