...
`merge.py`'s ancestor-aware hunk engine (`Hunk`, `MergeResult`, `merge_text`, `apply_action`, `Choice`) is gone; only `is_binary` remains. The daemon no longer tracks a current hunk, choices, or conflict state for a path — GET renders everything needed for the whole sequence up front, and POST only carries the final `submit`. 0024's "hunks use the selected right replica's stash" and "3-way merge... with a replica switcher" language is superseded by this decision. 0024's other decisions stand: the second localhost port and token auth, nav grouping by managed project, `same as hub` labeling, and apply force-overwriting every replica of a path. Stashed ancestor bytes and the out-of-sync reason clause are unrelated to the merge editor and are unchanged; the clause is now shown per replica in the list instead of once for a single selected replica.
