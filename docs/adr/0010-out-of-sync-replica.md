...
`daemon status` lists out-of-sync paths. `daemon start` and `reload` warn and ack; they do not block the daemon. 0.5.0 has no resolve shell: if the replica's bytes later match the hub, out-of-sync clears. Conflict resolution belongs in a later desktop UI; a bare CLI is a poor place to pick winners, so 0.5.0 is warnings plus manual file edits.
