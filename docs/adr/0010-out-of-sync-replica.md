...
`daemon status` lists out-of-sync paths. `daemon start` does not ask an isolation ack: the resolve UI is served by the running daemon. `reload` still warns and ack; it does not block the daemon. Out-of-sync also clears when the replica's bytes later match the hub. A bare CLI is a poor place to pick winners; the resolve UI is 0024. Sequential merge is 0025; Submit is resolve.
