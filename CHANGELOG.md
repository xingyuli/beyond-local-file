...
### Added
- **Live hub and fan-out** — The managed project is the hub. A mailbox coalesces per `(path, replica)`. The owner of a target path is the unique item (name equals the path or is a prefix) and its managed project — derived at runtime, not persisted. After a successful hub apply, that generation is copied or deleted onto other in-sync replicas of that managed project except the source — not every replica that happens to use the same item name.
- **No overlapping item paths** — Several managed projects may contribute to one target only if item names are disjoint (not equal, not a path prefix). `local-file` vs `local-file/devops/k8s.md` is a hard error on start/reload, naming both projects and both paths. Same rule inside one project's subpaths. Distinct siblings (`.vscode` and `.kiro/hooks`) are fine.
- **Out-of-sync replicas and held copies** — If two replicas edit the same path, the first apply wins; the loser is out-of-sync for that path (fan-out skips it; further edits from it are discarded). A delete past generation gap 3 still removes the live path and keeps previous hub bytes under `~/.blf/held/<sha256 of the managed project path>/`. `blf daemon status` lists both; `start` and `reload` warn and continue. 0.5.0 does not interview you to pick winners.
- **External YAML ingest** — Manual edits to a configuration set's mapping files apply only through `daemon start` or `daemon reload`. Removals print one plan and require confirmation; decline commits nothing. The daemon does not watch mapping files.
- **Daemon log timestamps** — Each new worker line in the set run directory's `daemon.log` is prefixed at write time with local timezone + offset, e.g. `2026-09-12T17:42:03+08:00 live: update alpha.txt gen 1`. `daemon logs` prints the file as stored. Old unstamped lines stay unstamped.
- **Revlink hub choice** — When several hubs target CWD and PATH is a new item, `revlink create` prompts for a managed project in the same command. No TTY lists the names and exits 1. One hub targeting CWD does not prompt. PATH already covered uses that owner. `revlink restore` and `remove` resolve the hub from PATH (contribution source), not CWD-level ambiguity; a path that is not a managed item is an error.

### Changed
- **Copy-only projections** — Every mapped file and directory is a physical copy at the projection path — that path is not a symlink to the hub. Nested symlink nodes inside a directory item (venv `python` → `python3.14` → the real interpreter) are copied as symlinks; following them is a bug. `link` remains the metaphor. Symlink is not a link strategy. Leftover blf symlinks that still point at the correct managed item become copies on first daemon catch-up. Windows Developer Mode is not required for ordinary copies; enable it when a tree contains nested links.
- **Daemon runtime** — One OS process per **configuration set**. The **global set** is the pointer list in `~/.blf/config` (replaces `~/.blfrc`). `-c PATH` is a **singleton set** identified by that file's resolved path. With neither, `config.yml` in the current directory is a singleton set. Resolution order: `-c` / `--config`, else the global config, else CWD `config.yml`. Pid, port, log, mapping snapshot, and baseline live under `~/.blf/run/global/` or `~/.blf/run/file-<sha256 of the resolved mapping yaml>/`. First start deletes leftover hub-local `.blf/` and prints each path. A mapping file already loaded by a running set is served by that process — `-c` does not start a second watcher. Starting a set that shares a mapping file with another running set is an error and names the owner. The process accepts IPC in both phases (`catch-up` and `ready`); live observation starts only in `ready`. `daemon start` stays in the foreground until `ready`. `status` always includes the pid. Other shells wait through catch-up. `link check` hashes managed vs target now; mismatch rows can be labeled from the baseline (managed-changed / target-changed / both-changed). There is no `sync-state.yml`. On a TTY, start and check rewrite one status line (`Catching up i/n` / `Checking i/n`), then print the table. Shells (`link check`, `revlink create` / `restore`, `remove`) send requests; if the daemon is down they fail with `Start it with: blf daemon start`.

### Removed
- **`blf link sync`** — There is no manual sync command; the daemon is the runtime.
- **`copy: true`** — Not a valid option. Config load rejects leftover flags with `Unsupported option 'copy: true' (project: …, mapping: …, key: copy)`.
- **`~/.blfrc`** — Not read. The global pointer list is `~/.blf/config`.

[0.5.0]: https://github.com/xingyuli/beyond-local-file/releases/tag/v0.5.0
