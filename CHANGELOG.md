...
- **Out-of-sync replicas and held copies** — If two replicas edit the same path, the first apply wins; the loser is out-of-sync for that path (fan-out skips it; further edits from it are discarded). A delete past generation gap 3 still removes the live path and keeps previous hub bytes under `.blf-held/` in the managed project. `blf daemon status` lists both; `start` and `reload` warn and continue. 0.5.0 does not interview you to pick winners.
- **External YAML ingest** — Manual `config.yml` edits apply only through `daemon start` or `daemon reload`. Removals print one plan and require confirmation; decline commits nothing. The daemon does not watch the config file.
- **Daemon log timestamps** — Each new worker line in `.blf/daemon.log` is prefixed at write time with local timezone + offset, e.g. `2026-09-12T17:42:03+08:00 live: update alpha.txt gen 1`. `daemon logs` prints the file as stored. Old unstamped lines stay unstamped.
...
- **Daemon runtime** — `blf daemon start|stop|status|logs|reload` is the process that copies, observes, and applies mapping changes. Shells (`link check`, `revlink create` / `restore`, `remove`) send requests; if the daemon is down they fail with `Start it with: blf daemon start`.
